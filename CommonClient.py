...
def _websocket_state_name(socket: typing.Any) -> str:
...
def _websocket_state_closed(socket: typing.Any) -> bool:
    return _websocket_state_name(socket) == "CLOSED"


def _websocket_state_open(socket: typing.Any) -> bool:
    state_name = _websocket_state_name(socket)
...
def _is_self_referential_ws_prop(socket: typing.Any, attr_name: str) -> bool:
    cls_dict = getattr(type(socket), "__dict__", {})
    prop = cls_dict.get(attr_name)
    if not isinstance(prop, property):
        return False
    getter = prop.fget
    if getter is None:
        return False
    if getattr(getter, "__module__", "") != __name__:
        return False
    # Guard against older lambda-based shim and this module's compat property getters.
    return getattr(getter, "__name__", "") in {"<lambda>", "_compat_closed_property", "_compat_open_property"}


def _websocket_closed_compat(socket: typing.Any) -> bool:
    if socket is None:
        return True

    closed_attr = None
    if not _is_self_referential_ws_prop(socket, "closed"):
    ...
    return _websocket_state_closed(socket)


def _websocket_open_compat(socket: typing.Any) -> bool:
    if socket is None:
        return False
    if _websocket_closed_compat(socket):
        return False

    open_attr = None
    if not _is_self_referential_ws_prop(socket, "open"):
    ...
    return _websocket_state_open(socket)


def _compat_closed_property(self: typing.Any) -> bool:
    return _websocket_state_closed(self)


def _compat_open_property(self: typing.Any) -> bool:
    return _websocket_state_open(self)


def _install_websocket_compat_shim() -> None:
    """Provide .open/.closed properties for websockets implementations that removed them."""
    classes: list[type] = []
    for mod_name in ("websockets.asyncio.client", "websockets.client"):
        try:
            mod = __import__(mod_name, fromlist=["ClientConnection"])
            cls = getattr(mod, "ClientConnection", None)
    ...
    for cls in classes:
        if not hasattr(cls, "closed"):
            try:
                setattr(cls, "closed", property(_compat_closed_property))
            except Exception:
                pass
        if not hasattr(cls, "open"):
            try:
                setattr(cls, "open", property(_compat_open_property))
            except Exception:
                pass


_install_websocket_compat_shim()


def websocket_is_closed(socket: typing.Any) -> bool:
    """Compatibility helper for websocket client implementations."""
    if socket is None:
        return True

    return _websocket_closed_compat(socket)


def websocket_is_open(socket: typing.Any) -> bool:
    return _websocket_open_compat(socket)
...
class CommonContext:
    ...
    async def disconnect(self, allow_autoreconnect: bool = False):
        ...
        if self.server and websocket_is_open(self.server.socket):
            await self.server.socket.close()
    ...
    async def send_msgs(self, msgs: typing.List[typing.Any]) -> None:
        ...
        if not websocket_is_open(socket):
            return
        await socket.send(encode(msgs))
    ...
    async def update_death_link(self, death_link: bool):
        ...
        if death_link:
            ...
            self.tags -= {"DeathLink"}
        if old_tags != self.tags and self.server and websocket_is_open(self.server.socket):
...
def handle_url_arg(args: "argparse.Namespace",
    ...
    if url.scheme != "archipelago" and url.scheme != "mwgg":
        ...
        parser.error(f"bad url, found {args.url}, expected url in form of archipelago://sekailink.xyz:38281 or mwgg://sekailink.xyz:38281")
        return args
