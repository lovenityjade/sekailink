...
def _update_firewall_port(port: int, allow: bool) -> None:
    if port <= 0:
        return
    if allow:
        cmd = ["sudo", "ufw", "allow", f"{port}/tcp"]
    else:
        cmd = ["sudo", "ufw", "--force", "delete", "allow", f"{port}/tcp"]
    try:
        subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=5)
    except Exception:
        logging.getLogger().exception("Failed to update UFW rule for port %s", port)
        ...
        with Locker(f"RoomLocker {room_id}"):
            ...
            try:
                ...
                if port:
                    ...
                    _update_firewall_port(port, True)
                    opened_port = port
                    with db_session:
                        room = Room.get(id=ctx.room_id)
                        room.last_port = port
                        try:
                            from .models import LobbyGeneration, LobbyMessage
                            from WebHostLib.lobbies import _add_system_message
                            from WebHostLib import socketio, to_url
                            lobby_gen = LobbyGeneration.get(room_id=room.id)
                            if lobby_gen:
                                message = f"Server: {host}:{port}"
                                existing = LobbyMessage.get(lobby=lobby_gen.lobby, content=message)
                                if not existing:
                                    _add_system_message(lobby_gen.lobby, message)
                                socketio.emit(
                                    "room_info",
                                    {
                                    ...
                                    to=f"lobby:{to_url(lobby_gen.lobby.id)}",
                                )
                        except Exception:
                            logger.exception("Failed to post lobby server info.")
                    del room
                else:
                ...
                try:
                    ...
                    if opened_port:
                        _update_firewall_port(opened_port, False)
