...
## Check
A check is a common term for when you "check", or pick up, a location. In terms of SekaiLink this is usually used for
...
## World
World in terms of SekaiLink can mean multiple things and is used interchangeably in many situations.
* During gameplay, a world is a single instance of a game, occupying one player "slot". However, 
SekaiLink allows multiple players to connect to the same slot; then those players can share a world 
...
* On the programming side, a world typically represents the package that integrates SekaiLink with a
particular game. For example this could be the entire `worlds/factorio` directory.

## RNG
Acronym for "Random Number Generator." SekaiLink uses its own custom Random object with a unique seed per generation,
...
## Seed
A "seed" is a number used to initialize a pseudorandom number generator. Whenever you generate a new game on SekaiLink
this is a new "seed" as it has unique item placement, and you can create multiple "rooms" on the SekaiLink site from a
single seed. Using the same seed results in the random placement being the same.

## Room
Whenever you generate a seed on the SekaiLink website you will be put on a seed page that contains all the seed info
...
## Logic
Base behavior of all seeds generated by SekaiLink is they are expected to be completable based on the requirements of
...
## Sphere
SekaiLink calculates the game playthrough by using a "sphere" system where it has a state for each player and checks
to see what the players are able to reach with their current items. Any location that is reachable with the current
state of items is a "sphere." For the purposes of SekaiLink it starts playthrough calculation by distributing sphere 0
