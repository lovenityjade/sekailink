...
function Expand-BundleArchive([string]$ZipPath, [string]$ExtractDir, [string]$BundledSevenZipPath) {
  if (![string]::IsNullOrWhiteSpace($BundledSevenZipPath) -and (Test-Path -LiteralPath $BundledSevenZipPath)) {
    Write-Step "Extracting bundle with bundled 7za"
    & $BundledSevenZipPath x "-o$ExtractDir" -y $ZipPath | Out-Null
    if ($LASTEXITCODE -ne 0) {
      throw "sevenzip_extract_failed:$LASTEXITCODE"
    }
    return
  }
  Write-Step "Extracting bundle with PowerShell"
...
function Remove-InstallContents([string]$TargetDir) {
  ...
  $keep = @(
    ...
    "SekaiLink-bootstrapper.cmd",
    "SekaiLink-bootstrapper.ps1",
...
$SevenZipPath = Default-String $SevenZipPath (Default-String $env:SEKAILINK_SEVENZIP_PATH "")
...
if ($needsUpdate) {
  ...
  try {
    ...
    Expand-BundleArchive $zipPath $extractDir $SevenZipPath
