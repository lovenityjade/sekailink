...
const createWebApClientRuntime = (deps = {}) => {
  ...
  const buildClientUrl = (baseUrl, serverAddress) => {
    ...
    if (server) url.hash = `server=${encodeURIComponent(server)}`;
    return url.toString();
  };
  ...
  const startWebApClient = async (options = {}) => {
    ...
    const clientUrl = buildClientUrl(options.url || options.clientUrl, serverAddress);
    ...
    win.webContents.on("did-finish-load", async () => {
      ...
      if (serverAddress) {
        setTimeout(() => {
          runCommand(clientId, `/connect ${serverAddress}`).catch((err) => {
