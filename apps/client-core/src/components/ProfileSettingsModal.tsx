...
import { apiCurrentUser, apiFetch, apiUrl } from "../services/api";
...
const ProfileSettingsModal: React.FC<ProfileSettingsModalProps> = ({ open, me, onClose }) => {
  ...
  const storageKey = useMemo(
    () => `skl_profile_settings_${me?.discord_id || me?.user_id || "guest"}`,
    [me?.discord_id, me?.user_id]
  );
  ...
  const loadFromServer = useCallback(async () => {
    setIsLoading(true);
    try {
      const [profileRes, patreonRes] = await Promise.all([
        apiFetch("/api/me/profile"),
        apiFetch("/api/me/linked-accounts/patreon"),
      ]);
      if (profileRes.ok || patreonRes.ok) {
        const data = profileRes.ok ? ((await profileRes.json()) as RemoteProfileResponse) : {};
        const patreonData = patreonRes.ok ? ((await patreonRes.json()) as RemoteProfileResponse) : {};
        const patreon = (patreonData.patreon || patreonData) as NonNullable<RemoteProfileResponse["patreon"]>;
        setSettings((prev) => ({
          displayName: (data.display_name || prev.displayName || me?.global_name || me?.username || "").trim(),
          bio: String(data.bio || ""),
          twitchUsername: String(patreon.tier || data.twitch_username || data.twitch_login || data.twitch_name || ""),
          twitchLinked: Boolean(
            patreon.linked
            || patreon.is_supporter
            || data.twitch_linked
      ...
      const meData = await apiCurrentUser();
      setSettings((prev) => ({
        ...prev,
        displayName: String(meData.global_name || meData.display_name || meData.username || prev.displayName || ""),
      }));
    } catch {
  ...
  const openPatreonLink = async () => {
    try {
      const response = await apiFetch("/api/me/linked-accounts/patreon/begin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ redirect_uri: "https://sekailink.com/settings/integrations/patreon/callback" }),
      });
      const data = await response.json().catch(() => null) as { authorization_url?: string } | null;
      const target = data?.authorization_url || "https://www.patreon.com/sekailink";
      ...
      const target = "https://www.patreon.com/sekailink";
      if (runtime.openExternal) {
        runtime.openExternal(target);
        return;
      }
  ...
  const unlinkPatreon = async () => {
    setSettings((prev) => ({ ...prev, twitchLinked: false, twitchUsername: "" }));
    await apiFetch("/api/me/linked-accounts/patreon/unlink", { method: "POST" }).catch(() => undefined);
  };
