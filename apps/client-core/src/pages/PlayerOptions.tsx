...
/** UTF-8 BOM (often added by Notepad / Excel) breaks many YAML parsers */
function stripYamlBom(text: string): string {
  return text.replace(/^\uFEFF/, "").replace(/\r\n/g, "\n");
}

/**
 * Archipelago templates use either a scalar or a weighted map { choice: weight }.
 * Pick the key with the highest strictly positive weight; ties → lexicographically smallest key.
 */
function resolveArchipelagoWeightedValue(raw: unknown): unknown {
  ...
  for (const [k, v] of entries) {
    ...
    if (w > bestW || (w === bestW && bestK !== null && k < bestK)) {
      bestW = w;
      bestK = k;
    }
  }
  if (bestK !== null && bestW > 0) return bestK;
  return raw;
}

function resolveGameField(raw: unknown, fallbackGame: string): string {
  const r = resolveArchipelagoWeightedValue(raw);
  if (typeof r === "string" && r.trim()) return r.trim();
  return fallbackGame;
}

const YAML_TOP_LEVEL_SKIP = new Set([
  "name",
  "game",
  "description",
  "requires",
...
/** Locate the per-game options object (handles alias keys like Pokemon_FireRed_and_LeafGreen). */
function findGameOptionsSection(parsed: Record<string, unknown>, canonicalGame: string): Record<string, unknown> {
  const tryKey = (k: string | undefined): Record<string, unknown> | null => {
    if (!k) return null;
    const v = parsed[k];
  ...
  const aliasKeys = [canonicalGame];
  if (canonicalGame === "Pokemon FireRed and LeafGreen") {
    aliasKeys.push("Pokemon_FireRed_and_LeafGreen");
  }
  for (const key of aliasKeys) {
    const found = tryKey(key);
  ...
  for (const [k, v] of Object.entries(parsed)) {
    if (YAML_TOP_LEVEL_SKIP.has(k)) continue;
...
const PlayerOptionsPage: React.FC<{ mode: PlayerOptionsMode }> = ({ mode }) => {
  ...
  const applyYaml = useCallback(
    (yamlText: string, payload: PlayerOptionsMeta) => {
      const text = stripYamlBom(yamlText);
      let parsed: any;
      try {
        ...
        setStatus(`${t("po.yaml_parse_failed")}: ${detail}`);
        return;
      }
      if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
        setStatus(t("po.yaml_invalid_format_detail"));
        return;
      }
      const parseRoot = parsed as Record<string, unknown>;
      const yamlGameResolved = resolveGameField(parseRoot.game, payload.game);
      const yamlOptions = findGameOptionsSection(parseRoot, payload.game);

      if (Object.keys(yamlOptions).length === 0) {
        setStatus(
          t("po.yaml_missing_game_section", {
            expectedGame: payload.game,
            gameField: yamlGameResolved
          })
        );
      } else {
        setStatus("");
      ...
      payload.groups.forEach((group) => {
        group.options.forEach((option) => {
          const raw = yamlOptions[option.name];
          if (raw === undefined) return;

          if (option.type === "option_set" || option.type === "option_list" || option.type === "item_set" || option.type === "location_set") {
            if (Array.isArray(raw)) {
              nextSets[option.name] = new Set(raw);
            }
            return;
          }
          ...
          const value = resolveArchipelagoWeightedValue(raw);
          if (value === "random") {
