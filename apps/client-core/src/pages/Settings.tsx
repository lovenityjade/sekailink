...
import { API_BASE_URL, apiFetch, getDesktopToken } from "../services/api";

type ImportResult = { gameId: string; file: string };
type RomEntry = { gameId: string; path: string };
type GameAutomationStatusCard = {
  label: string;
  className: "ready" | "missing";
  note: string;
  correctionLabel?: string;
  correctionTab?: "paths" | "roms";
};
...
const SettingsPage: React.FC = () => {
  ...
  const [gameConfigVersion, setGameConfigVersion] = useState(0);
  const [automationStatusByModule, setAutomationStatusByModule] = useState<Record<string, GameAutomationStatusCard>>({});
  ...
  const [sekaiemuExePath, setSekaiemuExePath] = useState("");
  ...
  const [settingsTab, setSettingsTab] = useState<"general" | "video" | "audio" | "paths" | "roms" | "linux" | "social" | "blocks">("general");
  ...
  const [blockedUsers, setBlockedUsers] = useState<Array<{ user_id: string; display_name: string; avatar_url?: string; created_at?: string }>>([]);
  const [socialStatus, setSocialStatus] = useState("");
  type SettingsTab = "general" | "video" | "audio" | "paths" | "roms" | "linux" | "social" | "blocks";
  ...
  const refreshConfig = async () => {
    ...
    const sekaiemu = games?.sekaiemu || {};
    setSekaiemuExePath(typeof sekaiemu?.exe_path === "string" ? sekaiemu.exe_path : "");
    ...
    setGameConfigVersion((prev) => prev + 1);
  };
      ...
      setBlockedUsers(
        list
          .map((entry: any) => ({
            ...
            display_name: String(entry?.display_name || entry?.user_id || "").trim(),
  ...
  useEffect(() => {
    ...
    const toAutomationStatusCard = (readiness: LaunchReadiness): GameAutomationStatusCard => {
      if (readiness.ready) {
        return {
          label: "Ready",
          className: "ready",
          note: readiness.trackerNeeded ? "Core launch checks are satisfied for this game." : "Automation setup is ready.",
        };
      }
      ...
      if (err === "rom_missing") {
        return {
          label: "ROM Required",
          className: "missing",
          note: "Import the base game file before this runtime can launch.",
          correctionLabel: "Open ROM Library",
          correctionTab: "roms",
        };
      }
      ...
      if (err === "iwad_missing") {
        return {
          label: "IWAD Required",
          className: "missing",
          note: "gzDoom still needs an IWAD path in this section.",
          correctionLabel: "Review gzDoom Setup",
          correctionTab: "paths",
        };
      }

      if (err === "gzap_pk3_missing") {
        return {
          label: "gzArchipelago Required",
          className: "missing",
          note: "gzDoom still needs a gzArchipelago.pk3 path in this section.",
          correctionLabel: "Review gzDoom Setup",
          correctionTab: "paths",
        };
      }

      if (err === "sm64ex_not_found") {
        return {
          label: "SM64EX Required",
          className: "missing",
          note: "Set an SM64EX executable path or root directory before launch.",
          correctionLabel: "Review SM64EX Setup",
          correctionTab: "paths",
        };
      }

      if (err === "soh_not_found" || err === "soh_install_failed") {
        return {
          label: "SoH Required",
          className: "missing",
          note: "Set a Ship of Harkinian executable or enable auto-install.",
          correctionLabel: "Review SoH Setup",
          correctionTab: "paths",
        };
      }
      ...
      return {
        ...
        className: "missing",
        note: "This runtime still needs additional setup before launch.",
        correctionLabel: String(readiness.setupArea || "").trim().toLowerCase() === "roms" ? "Open ROM Library" : "Review Game Setup",
        correctionTab: String(readiness.setupArea || "").trim().toLowerCase() === "roms" ? "roms" : "paths",
      };
    };

    const refreshAutomationStatuses = async () => {
      if (settingsTab !== "paths") return;

      const moduleIds = ["factorio", "gzdoom", "sm64ex", "oot_soh"];
      const entries = await Promise.all(
        moduleIds.map(async (moduleId) => {
          const readiness = await getLaunchReadiness(moduleId).catch(
            () =>
              ({
                ok: false,
                ready: false,
          ...
          return [moduleId, toAutomationStatusCard(readiness)] as const;
        })
      );

      if (cancelled) return;
      setAutomationStatusByModule(Object.fromEntries(entries));
    };

    void refreshAutomationStatuses();

    return () => {
      cancelled = true;
    };
  }, [settingsTab, gameConfigVersion]);

  const renderAutomationStatus = (moduleId: string) => {
    const card = automationStatusByModule[moduleId];
    if (!card) return null;
    return (
      <div className="skl-ready-status" style={{ marginTop: 10 }}>
        <strong style={{ color: card.className === "ready" ? "rgba(0,255,200,0.88)" : "rgba(255,206,138,0.92)" }}>{card.label}</strong>
        <span style={{ marginLeft: 8 }}>{card.note}</span>
        {card.correctionLabel && (
          <button
            className="skl-btn ghost"
            type="button"
            style={{ marginLeft: 10 }}
            onClick={() => setSettingsTab(card.correctionTab || "paths")}
          >
            {card.correctionLabel}
          </button>
        )}
  ...
  const unblockUser = async (userId: string) => {
    setSocialStatus("");
    try {
      const res = await apiFetch("/api/social/blocks/remove", {
        method: "POST",
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({} as any));
        setSocialStatus(String(data?.error || "Unable to unblock user."));
        return;
      }
      setSocialStatus("User unblocked.");
      await loadBlockedUsers();
    } catch (err) {
      setSocialStatus(err instanceof Error ? err.message : "Unable to unblock user.");
    }
  };
  ...
  const runImport = async () => {
    ...
    const pick = await runtime.pickFile?.({
      ...
      filters: [{ name: "ROM", extensions: ["gba", "gb", "gbc", "sfc", "smc", "z64", "n64", "nes"] }]
    });
  ...
  const saveSekaiemu = async () => {
    setGamesStatus("");
    try {
      const patch: any = {
        exe_path: sekaiemuExePath.trim() || undefined,
    ...
    <div className="skl-game-manager skl-app-panel skl-settings-page">
      ...
      <div className="skl-gm-tabs skl-settings-tabs" role="tablist" aria-label={t("settings.title")} data-tutorial="settings-tabs">
        {([
          ...
          ["blocks", "Blocked users", "M7 7l10 10M17 7L7 17"],
      ...
      <div className="skl-gm-panel active" role="tabpanel" style={{ marginTop: 16 }}>
        ...
        <div className="yaml-dashboard-card">
          ...
          {renderAutomationStatus("factorio")}
          ...
          <details style={{ marginTop: 12 }}>
            <summary style={{ cursor: "pointer", fontWeight: 700 }}>Sekaiemu Libretro Runtime</summary>
            <div style={{ marginTop: 10 }}>
              <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 10 }}>
                Used by SekaiLink Core for BETA-3 native emulator handoff. Core prepares the seed image, then Sekaiemu boots it with the declared libretro core.
              </div>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                <label style={{ display: "flex", flexDirection: "column", gap: 6, flex: "1 1 420px" }}>
                  Executable path
                  <div style={{ display: "flex", gap: 8 }}>
                    <input className="input" value={sekaiemuExePath} onChange={(e) => setSekaiemuExePath(e.target.value)} style={{ flex: 1 }} />
                    <button
                      className="skl-btn ghost"
                      type="button"
                      onClick={async () => {
                        const pick = await runtime.pickFile?.({ title: "Select Sekaiemu executable" });
                        if (pick && !pick.canceled && pick.path) setSekaiemuExePath(pick.path);
                      }}
                    >
                      Browse
                    </button>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 12 }}>
                <label style={{ display: "flex", flexDirection: "column", gap: 6, flex: "1 1 420px" }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <input className="input" value={sekaiemuRootDir} onChange={(e) => setSekaiemuRootDir(e.target.value)} style={{ flex: 1 }} />
                    <button
                      className="skl-btn ghost"
                      type="button"
                      onClick={async () => {
                        const pick = await runtime.pickFolder?.({ title: "Select Sekaiemu root directory" });
                        if (pick && !pick.canceled && pick.path) setSekaiemuRootDir(pick.path);
                      }}
                    >
                      Browse
                    </button>
              ...
              <div style={{ marginTop: 12 }}>
                <div style={{ fontSize: 12, opacity: 0.8, marginBottom: 6 }}>Libretro core directories (one per line):</div>
                <textarea
                  className="input"
                  value={sekaiemuCoreDirsText}
              ...
              <div style={{ marginTop: 12 }}>
                <div style={{ fontSize: 12, opacity: 0.8, marginBottom: 6 }}>Extra Sekaiemu args (one per line):</div>
                <textarea
                  className="input"
                  value={sekaiemuArgsText}
                  onChange={(e) => setSekaiemuArgsText(e.target.value)}
                  style={{ width: "100%", minHeight: 70, fontFamily: "monospace" }}
                  placeholder={"Example:\n--probe-only"}
                />
              </div>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 12 }}>
                <button className="skl-btn primary" type="button" onClick={saveSekaiemu}>
                  Save Sekaiemu
                </button>
              </div>
            </div>
          ...
          <details style={{ marginTop: 12 }}>
            ...
            <div style={{ marginTop: 10 }}>
          {renderAutomationStatus("oot_soh")}
          <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 10 }}>
      ...
      <div className="skl-gm-panel active" role="tabpanel" style={{ marginTop: 16 }}>
        ...
        <div className="yaml-dashboard-card">
          ...
          <p style={{ opacity: 0.8, marginTop: 8 }}>
            Manage blocked users in the <strong>Blocked users</strong> tab.
          </p>
        </div>
      </div>
      )}
      ...
      <div className="skl-gm-panel active" role="tabpanel" style={{ marginTop: 16 }}>
        ...
        <div className="yaml-dashboard-card">
          <p style={{ opacity: 0.8 }}>
            Blocked users are hidden from social and online lists, and direct messages are disabled both ways.
          </p>
          <div style={{ marginTop: 10, maxHeight: 320, overflow: "auto", border: "1px solid rgba(56, 243, 221, 0.16)", borderRadius: 10, padding: 8 }}>
            {blockedUsers.length === 0 && (
              <div className="skl-ready-status">No blocked users.</div>
            )}
            {blockedUsers.map((entry) => (
              <div key={entry.user_id} className="yaml-row" style={{ marginBottom: 8 }}>
                <div className="yaml-meta" style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span className="sklp-friend-avatar" style={{ width: 26, height: 26 }}>
                    {entry.avatar_url ? <img src={entry.avatar_url} alt="" /> : null}
                  </span>
                  <div>
                    <div className="yaml-title">{entry.display_name}</div>
                    <div className="yaml-sub">{entry.user_id}</div>
                  </div>
                </div>
                <button className="skl-btn" type="button" onClick={() => void unblockUser(entry.user_id)}>
                  Unblock
                </button>
              </div>
            ))}
          </div>
          {socialStatus && <div className="skl-ready-status" style={{ marginTop: 8 }}>{socialStatus}</div>}
        </div>
      </div>
      )}
    </div>
