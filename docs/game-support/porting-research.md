...
| `bizhawk_generic_lua` | APWorld setup expects Archipelago's generic BizHawk Lua protocol. | Medium risk after native absorption. Sekaiemu/SKLMI now implements the protocol shape natively, so the remaining work is per-core domain parity, generated manifests, and live game certification. |
...
2. Finish and certify the native Sekaiemu/SKLMI absorption of the generic
   BizHawk Lua contract for `bizhawk_generic_lua` games.
...
| Mega Man 2 | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol, `.apmm2` patch, NES domains. | Needs live NES domain certification. |
| Mega Man 3 | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol, `.apmm3` patch, NES domains. | Tracker is non-blocking; good first live candidate. |
| Super Mario Land 2 | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol, `.apsml2` patch, GB domains. | Platformer; good first live candidate if GB domains resolve cleanly. |
| Pokemon Emerald | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol, `.apemerald` patch, GBA domains. | Strong target after one smaller generic game passes. |
| Pokemon Red and Blue | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol and GB domains. | PopTracker exists; runtime challenge is domain parity. |
| Pokemon Crystal | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol and GBC domains. | Needs public setup URL confirmation. |
| Pokemon FireRed and LeafGreen | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol and GBA domains. | Similar bridge class as Emerald. |
| Metroid: Zero Mission | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol and GBA domains. | GBA domain certification required. |
| The Legend of Zelda | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol and NES domains. | NES domain certification required. |
| Oracle of Ages / Oracle of Seasons | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol and GBC domains, `.apoos` for Seasons evidence. | Verify Ages/Seasons split before automation. |
| Wario Land / Wario Land 4 | `bizhawk_generic_lua` | Medium | BizHawk-generic protocol and GB/GBA domains. | Platformer category; live domain proof still required. |
...
- `bizhawk_generic_lua` games should be batched behind one native
  Sekaiemu/SKLMI contract-certification project. The Lua protocol is reference
  behavior, not the runtime transport.
...
- Native generic BizHawk contract absorption in
