...
| `bizhawk_generic_lua` | APWorld uses Archipelago BizHawk Client plus `data/lua/connector_bizhawk_generic.lua`. | Higher risk. Sekaiemu must emulate or replace the BizHawk memory-domain bridge before SKLMI can observe/inject generically. |
...
2. Build a generic BizHawk Lua compatibility bridge inside Sekaiemu for
...
| Mega Man 2 | `bizhawk_generic_lua` | High | BizHawk 2.7+, `.apmm2` patch, BizHawk Client, `connector_bizhawk_generic.lua`. | NES support depends on generic BizHawk bridge work. |
| Mega Man 3 | `bizhawk_generic_lua` | High | BizHawk 2.7+, `.apmm3` patch, BizHawk Client, `connector_bizhawk_generic.lua`. | Tracker is non-blocking; runtime still needs BizHawk bridge. |
| Super Mario Land 2 | `bizhawk_generic_lua` | High | BizHawk 2.9.1 recommended, `.apsml2` patch, BizHawk Client, `connector_bizhawk_generic.lua`. | Platformer, but the client bridge is still BizHawk. |
| Pokemon Emerald | `bizhawk_generic_lua` | High | BizHawk 2.7+, `.apemerald` patch, BizHawk Client, `connector_bizhawk_generic.lua`. | Good long-term target once the generic bridge exists. |
| Pokemon Red and Blue | `bizhawk_generic_lua` | High | BizHawk Client and `connector_bizhawk_generic.lua`. | PopTracker exists; runtime challenge is bridge parity. |
| Pokemon Crystal | `bizhawk_generic_lua` | High | BizHawk Client path from local APWorld evidence. | Needs public setup URL confirmation. |
| Pokemon FireRed and LeafGreen | `bizhawk_generic_lua` | High | BizHawk Client and generic Lua connector from local setup. | Similar bridge class as Emerald. |
| Metroid: Zero Mission | `bizhawk_generic_lua` | High | BizHawk Client and generic Lua connector from local setup. | GBA bridge work required. |
| The Legend of Zelda | `bizhawk_generic_lua` | High | BizHawk Client from local APWorld evidence. | NES bridge work required. |
| Oracle of Ages / Oracle of Seasons | `bizhawk_generic_lua` | High | BizHawk 2.10 style path, `.apoos` for Seasons, BizHawk Client. | Verify Ages/Seasons split before automation. |
| Wario Land / Wario Land 4 | `bizhawk_generic_lua` | High | BizHawk Client and generic Lua connector from local setup. | Platformer category, but still BizHawk bridge. |
...
- `bizhawk_generic_lua` games should be batched behind one generic compatibility
  project. Once that bridge is proven, many NES/GB/GBC/GBA games become much
  cheaper together.
