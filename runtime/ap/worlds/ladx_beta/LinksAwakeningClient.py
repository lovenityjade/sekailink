...
class LAClientConstants:
    ...
    # The 13.x beta ROM can sit in normal gameplay states above the older
    # client cap. Keeping this at 0x1A leaves the client connected to AP but
    # forever "not game ready", so checks/items never reach the server.
    MaxGameplayValue = 0x1E
...
class LinksAwakeningClient():
    ...
    async def main_tick(self, ctx, item_get_cb, win_cb, death_link_cb):
        ...
        recv_keys = tuple(sorted(ctx.recvd_checks.keys()))
        debug_state = (recv_index, wMWCommand, recv_keys[-5:])
        if debug_state != self.last_recv_debug_state:
            self.last_recv_debug_state = debug_state
            logger.info(
                f"LADX receive state: rom_index={recv_index} command={hex(wMWCommand)} "
                f"queued_indexes={list(recv_keys[-5:])}"
            )
        if recv_index in ctx.recvd_checks:
            item = ctx.recvd_checks[recv_index]
            logger.info(
                f"LADX injecting item: index={recv_index} item={item.item} "
                f"sender={item.player}"
            )
...
class LinksAwakeningContext(CommonContext):
    ...
    def on_package(self, cmd: str, args: dict):
        ...
        if cmd == "ReceivedItems":
            logger.info(
                f"LADX ReceivedItems: index={args.get('index')} count={len(args.get('items', []))}"
            )
