...
class LocationTracker:
    ...
    def has_start_item(self):
        has_item = self.start_check not in self.remaining_checks
        if not has_item:
            logger.debug(
                "LADX start item gate blocked: id=%s address=0x%04X mask=0x%02X value=%s",
                self.start_check.id,
                self.start_check.address,
                self.start_check.mask,
                self.start_check.value,
            )
        return has_item
