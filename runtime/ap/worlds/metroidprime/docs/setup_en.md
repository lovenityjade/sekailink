# Setup Guide for Metroid Prime SekaiLink

This guide is meant to help you get up and running with Metroid Prime APWorld with SekaiLink.
This has only been tested on Windows, but feel free to let us know if you get the chance to try it on other OS platforms!

## Requirements

The following are required in order to play _Metroid Prime_ in SekaiLink:
...
3. In the SekaiLink Launcher, select `Install APWorld`, and then select `metroidprime.apworld` file from the previous step.

>[!NOTE]
> Because SekaiLink 5.1 on Windows transitioned to using Python 3.12, [Metroid Prime AP version 0.4.8](https://github.com/Electro1512/MetroidAPrime/releases/tag/v0.4.8) has two versions:
> | SekaiLink Version (Windows) |                                              |
> |-----------------------------------|----------------------------------------------|
...
>
>  Future versions after Metroid Prime AP 0.4.8 will likely target only Python 3.12 and will only work on SekaiLink 5.1.

>[!IMPORTANT]
> If you have used a previous version of Metroid Prime AP that required copying folders into the `/lib` folder, go to your `SekaiLink/lib` folder and delete the following directories:
...
As usual, randomized SekaiLink games with custom worlds must be generated locally - see [SekaiLink Setup Guide: Generating a game - On your local installation](https://sekailink.com/tutorial/Archipelago/setup/en#on-your-local-installation)

## Hosting a Room

If you're generating the multiworld, follow the instructions in the previous section.
Once you have the zip file corresponding to your multiworld, follow [SekaiLink Setup Guide: Hosting a SekaiLink Server](https://sekailink.com/tutorial/Archipelago/setup/en#hosting-an-archipelago-server) to host a room.

> [!NOTE]
> When hosting with the SekaiLink website, the website will *not* host patch files from imported custom worlds, such as Metroid Prime AP.
...
1. In the SekaiLink Launcher, click `Open Patch File`. Then select the `.apmp1` patch file.
...
5. After the game is running, connect the Metroid Prime Client to the room by entering the server name and port number at the top and pressing `Connect`.
   For rooms hosted on the website, this will be `sekailink.com:<port>`, where `<port>` is the port number.
   If a game is hosted from the `SekaiLinkServer.exe` (without `.exe` on Linux), this will default to `38281` but may be changed in the `host.yaml`.

>[!TIP]
>  **Optional**
>  If you want double-clicking `.apmp1` patch file to automatically open your game for you,
>    - Navigate to your `SekaiLink` installation and edit the `host.yaml` file.
...
- Use the latest Metroid Prime SekaiLink release
  - Metroid Prime SekaiLink: [Releases · Electro1512_MetroidAPrime](https://github.com/Electro1512/MetroidAPrime/releases)
...
- If you do not see the client in the SekaiLink Launcher
  ...
  - Go to `%TEMP%` (C:\Users\<Username>\AppData\Local\Temp) and delete the folder `ap_metroidprime_temp_lib_vX.Y.Z`. Then restart the SekaiLink Launcher.
...
- I have the randomized game open in Dolphin, but the Metroid Prime client says it can't connect to it!
  - Make Sure the ISO is Randomized
    - On the Main Menu, "SekaiLink Metroid Prime" text should appear. ([image example](https://i.imgur.com/W6172zf.png))
