...
### What you'll Need
- The latest release of [SekaiLink](https://github.com/SekaiLink/SekaiLink/releases/latest)
...
- The Skyward Sword AP World:
    - This includes the [APWorld file and the YAML options file](https://github.com/Battlecats59/SS_APWorld/releases/latest) (not needed for SekaiLink)
...
### Useful Links
- [SekaiLink Setup Guide](/tutorial/Archipelago/setup/en)
...
### Setting up Archipelago (not necessary with SekaiLink)
...
- After opening the randomized iso in Dolphin, open the `Skyward Sword Client` in SekaiLink.
...
- Connect to the room in your client by running `/connect {address}`. The link to the room should be given to you by the multiworld host. The address will be in the form of `sekailink.com:XXXXX`.
...
- After opening the randomized iso, open the `Skyward Sword Client` in SekaiLink.
    - Make sure your in game hash (on the top of the file select screen) is the same that the randomizer gave you when you generated the iso. It should be `AP P{player id} {three random words}`.
    - If your client does not open, follow the steps listed [here](https://github.com/Battlecats59/SS_APWorld/releases/tag/DME) to make sure the DME package is installed.
...
- [Skyward Sword Archipelago Web Tracker](https://youraveragelink.github.io/SSR-AP-Tracker) (created by YourAverageLink and robojumper)
    - This is the recommended tracker for multiworld via SekaiLink, since it will auto track items and locations.
...
For information on generating an AP world, visit the [SekaiLink Setup Guide](/tutorial/Archipelago/setup/en#generating-a-multiplayer-game).

### Running from Source
SekaiLink requires python 3.13.11 or newer (not the Windows Store version) and Git
...
- [Running from source on Windows](https://github.com/SekaiLink/SekaiLink/blob/main/docs/running%20from%20source.
