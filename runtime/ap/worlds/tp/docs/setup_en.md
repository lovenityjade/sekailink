# Setup Guide for Twilight Princess SekaiLink

Welcome to Twilight Princess SekaiLink! This guide will help you set up the randomizer and play your first multiworld.
Whether playing, generating, or hosting a SekaiLink room with Twilight Princess, you must follow a few simple steps to
get started.
...
- Install [SekaiLink](https://github.com/SekaiLink/SekaiLink/releases) v0.6.1 or higher.\
...
- Grab the `tar.gz` version of SekaiLink, not the `AppImage`. The file name should be similar to the following on the
  release page: `SekaiLink.X.X_linux-x86_64.tar.gz`.
...
1. Gather all player's YAMLs. Place these YAMLs into the `Players` folder of your SekaiLink installation. If the
   folder does not exist, then it must be created manually. The files here should not be compressed.
2. Modify any local host settings for generation, as desired.
3. Run `SekaiLinkGenerate.exe` (without `.exe` on Linux) or click `Generate` in the launcher. The generation output
...
[these steps](https://sekailink.com/tutorial/Archipelago/setup/en#hosting-an-archipelago-server) to host a room. Follow
...
7. Start `SekaiLinkLauncher.exe` (without `.exe` on Linux) and choose `Twilight Princess Client`, which will open the
...
9. Connect to the room by entering the server name and port number at the top and pressing `Connect`. For rooms hosted
   on the website, this will be `sekailink.com:<port>`, where `<port>` is the port number. If a game is hosted from the
   `SekaiLinkServer.exe` (without `.exe` on Linux), this will default to `38281` but may be changed in the `host.yaml`.

## Troubleshooting

- Ensure that you are running version v0.6.1 or higher of SekaiLink, and the latest version of the world.
- If you do not see the client in the launcher, ensure the `Twilight Princess.apworld` file is in the correct folder (the
  `custom_worlds` folder of your SekaiLink installation).
