# Setup Guide for Paper Mario: The Thousand-Year Door SekaiLink

## Required Software

- Dolphin Emulator: [Dolphin Emulator Releases](https://dolphin-emu.org/download/?ref=btn)
- SekaiLink: [Latest releases](https://github.com/SekaiLink/SekaiLink/releases)
...
Once you've installed the apworld, you can generate a yaml using the `Generate Template Options` button in the SekaiLinkLauncher. 
...
Once you have done so, run the `SekaiLinkGenerate.exe` application, and your game will be placed in the `output` folder if it was successful.
...
An alternative method is to double click the patch, and select `SekaiLinkLauncher.exe` as the application to run it with.
...
### Hosting a game
Any `.zip` file you generate can be uploaded [here](https://sekailink.com/uploads) to create a room, which you can join in your client.
...
_Note for Mac users: Due to additional security hardening that Apple added to MacOS, the steps from [here](https://github.com/aldelaro5/dolphin-memory-engine?tab=readme-ov-file#macos-code-signing) relating to Code Signing must be performed. Not doing so will prevent your SekaiLink Client from connecting to the game appropriately._