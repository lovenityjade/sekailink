...
std::string read_line(const socket_handle client) {
    ...
    while (true) {
        ...
        if (received < 0) {
            throw std::runtime_error("socket_read_failed");
        }
        if (ch == '\n') {
            break;
        }
...
void write_all(const socket_handle client, const std::string& payload) {
    ...
    while (sent < payload.size()) {
        ...
        if (count <= 0) {
            throw std::runtime_error("socket_write_failed");
        }
...
nlohmann::json process_request(GbaRuntime& runtime, const nlohmann::json& req) {
    ...
    if (type == "SYSTEM") {
        return {{"type", "SYSTEM_RESPONSE"}, {"value", runtime.system()}};
    }
    if (type == "PREFERRED_CORES") {
        return {{"type", "PREFERRED_CORES_RESPONSE"}, {"value", nlohmann::json::object()}};
    }
    if (type == "HASH") {
        return {{"type", "HASH_RESPONSE"}, {"value", runtime.rom_hash()}};
    }
    if (type == "MEMORY_SIZE") {
    ...
    if (type == "LOCK") {
        runtime.set_locked(true);
        return {{"type", "LOCKED"}};
    }
    if (type == "UNLOCK") {
        runtime.set_locked(false);
    ...
    if (type == "DISPLAY_MESSAGE") {
        runtime.display_message(req.value("message", ""));
        return {{"type", "DISPLAY_MESSAGE_RESPONSE"}};
    }
    if (type == "SET_MESSAGE_INTERVAL") {
        runtime.set_message_interval(req.value("value", 0.0));
...
std::string handle_line(GbaRuntime& runtime, const std::string& line) {
    if (line == "VERSION") {
        return std::to_string(kScriptVersion) + "\n";
    }

    const auto requests = nlohmann::json::parse(line);
    if (!requests.is_array()) {
        throw std::runtime_error("request_must_be_array");
    }

    nlohmann::json responses = nlohmann::json::array();
    std::optional<nlohmann::json> failed_guard;

    runtime.with_transaction([&]() {
        for (const auto& req : requests) {
            if (failed_guard.has_value()) {
                responses.push_back(*failed_guard);
                continue;
            }
            try {
                auto response = process_request(runtime, req);
                if (response.value("type", "") == "GUARD_RESPONSE" && !response.value("value", false)) {
...
void serve_forever(GbaRuntime& runtime, const Args& args) {
    init_sockets();
    ...
    std::cout << "[sekailink-runtime-gba] listening on " << args.host << ":" << args.port
              << " protocol=ULCPv1 system=GBA rom_hash=" << runtime.rom_hash();
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::cerr << "[sekailink-runtime-gba] fatal: " << ex.what() << '\n';
        return 1;
    }
}
