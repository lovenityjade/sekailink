...
function Class:extend(obj)
    ...
    mt.__index = function(table, key)
        local val = rawget(table.item, key)
    ...
    mt.__newindex = function(table, key, value)
        local val = rawget(table.item, key)
        if val and type(val) == "table" and ((val.set ~= nil and val.item == nil) or val.value ~= nil) then
            local v = value
            if val.set then
...
function Class:set(prop, value)
    if not value and type(prop) == "table" then
        for k, v in pairs(prop) do
            rawset(self.item, k, v)
        end
    else
        rawset(self.item, prop, value)
    end
end
