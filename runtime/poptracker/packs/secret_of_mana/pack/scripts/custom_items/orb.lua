...
function OrbItem:init(name, code)
    ...
    self.isSpriteDisabled = false
    self.isGirlDisabled = false
    self.isRoleRando = false
    self.hasAllElements = false
    self.hasGirlRole = false
    self.hasSpriteRole = false
    self.state = 0
    self.notNeeded = {}
...
function OrbItem:advanceState()
    ...
    if self.isSpriteDisabled and self.isGirlDisabled then
        if (self.state ~= 0 and not self.isRoleRando) then
            self:setState(0)
        end
        return
    end
...
function OrbItem:propertyChanged(key, value)
    ...
    if key == "isSpriteDisabled" then
        ...
        if self:isStateDisabled(self.state) and not self.isRoleRando then
            self:setState(0)
        end
    end
    ...
    if (key == "isSpriteDisabled" or key == "isGirlDisabled") and self.isGirlDisabled and self.isSpriteDisabled and
        not self.isRoleRando then
        self:setState(0)
    end
