...
int main() {
  try {
    RoomSession session(RoomSessionConfig{
        ...
        .seed_name = "seed-projection-name-1",
        .seed_id = "seed-123",
        .seed_hash = "hash-abc",
    ...
    session.heartbeat_runtime({
        .runtime_kind = "sklmi",
        .runtime_session_name = "projection-live-1",
        .driver_instance_id = "driver-projection-1",
        .linkedworld_id = "alttp",
        .core_profile = "snes_v1",
        .connected = true,
    });
    session.set_slot_data({{"difficulty", "normal"}, {"goal", "ganon"}});
    ...
    const auto delta_batch = build_projection_batch(
        session.snapshot(),
        session.events(),
        session.client_reports(),
        session.events().size() - 1,
        session.client_reports().size() - 1);
    ...
    require(batch.room_record["seed_name"] == "seed-projection-name-1", "seed_name");
    require(batch.room_record["slot_data_keys"].size() == 2, "slot_data_keys");
    require(batch.room_record["latest_received_item"]["item_name"] == "Bow", "latest_received_item");
    require(batch.room_record["next_received_item_index"] == 1, "next_received_item_index");
    require(batch.room_record["runtime_connected"] == true, "runtime_connected");
    require(batch.room_record["driver_instance_id"] == "driver-projection-1", "driver_instance_id");
    require(delta_batch.room_event_records.size() == 1, "delta_event_records");
    require(delta_batch.client_report_records.size() == 1, "delta_report_records");
