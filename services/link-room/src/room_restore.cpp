...
ReceivedItemView received_item_from_json(const nlohmann::json& value) {
  return ReceivedItemView{
      ...
      .event_key = optional_value<std::string>(value, "event_key"),
      .mapped_value = optional_value<std::string>(value, "mapped_value"),
      .tracker_semantic_id = optional_value<std::string>(value, "tracker_semantic_id"),
  };
}

RuntimeBridgeState runtime_bridge_state_from_json(const nlohmann::json& value) {
  return RuntimeBridgeState{
      .runtime_kind = optional_value<std::string>(value, "runtime_kind"),
      .runtime_session_name = optional_value<std::string>(value, "runtime_session_name"),
      .driver_instance_id = optional_value<std::string>(value, "driver_instance_id"),
      .linkedworld_id = optional_value<std::string>(value, "linkedworld_id"),
      .core_profile = optional_value<std::string>(value, "core_profile"),
      .client_name = optional_value<std::string>(value, "client_name"),
      .client_version = optional_value<std::string>(value, "client_version"),
      .session_token_hash = optional_value<std::string>(value, "session_token_hash"),
      .ticket_issued_at = optional_value<std::string>(value, "ticket_issued_at"),
      .last_heartbeat_at = optional_value<std::string>(value, "last_heartbeat_at"),
      .last_item_delivery_at = optional_value<std::string>(value, "last_item_delivery_at"),
      .last_check_report_at = optional_value<std::string>(value, "last_check_report_at"),
      .last_delivery_ack_at = optional_value<std::string>(value, "last_delivery_ack_at"),
      .last_disconnect_reason = optional_value<std::string>(value, "last_disconnect_reason"),
      .last_delivery_ack_id = optional_value<std::int64_t>(value, "last_delivery_ack_id"),
      .acknowledged_delivery_ids = value.value("acknowledged_delivery_ids", std::vector<std::int64_t>{}),
      .heartbeat_count = value.value("heartbeat_count", static_cast<std::uint64_t>(0)),
      .connected = value.value("connected", false),
  };
}
...
RoomStateSnapshot room_state_snapshot_from_json(const nlohmann::json& value) {
  ...
  snapshot.seed_name = optional_value<std::string>(value, "seed_name");
  ...
  snapshot.linkedworld_surface = value.value("linkedworld_surface", nlohmann::json::object());
  snapshot.seed_contract = value.value("seed_contract", nlohmann::json::object());
  ...
  if (value.contains("runtime_state") && !value.at("runtime_state").is_null()) {
    snapshot.runtime_state = runtime_bridge_state_from_json(value.at("runtime_state"));
  }
