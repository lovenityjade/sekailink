...
int main() {
  ...
  const auto manifest_path = std::filesystem::temp_directory_path() / "sekailink-chat-api-release-smoke.json";
  {
    ...
    out << R"({"version":"0.3.1-smoke","channel":"test","platform":"linux-x64","download_url":"https://sekailink.example/client.AppImage","sha256":"abc123"})";
  }
  ...
  if (release.status != 200 || release.body.find("0.3.1-smoke") == std::string::npos || release.body.find("abc123") == std::string::npos) {
