...
constexpr std::uint64_t kReadSnapshotMaxBytes = 4096;
constexpr std::size_t kMaxRoomChatMessages = 64;

std::string DescribeRoomMetadataSnapshot(const std::unordered_map<std::string, std::string>& metadata) {
...
std::string DescribeInjectionRule(const InjectRule& rule) {
    std::ostringstream detail;
    detail << "{item_id=" << rule.item_id
           ...
           << ",mode=" << (rule.writes.empty() ? "direct" : "write_sequence")
           << "}";
    return detail.str();
}

std::string DescribeRoomItem(const RoomItem& item) {
    std::ostringstream detail;
...
std::string BuildRoomChatMessagesJson(const std::vector<RoomChatMessage>& messages) {
...
std::string RoomReportKey(const Event& event) {
...
bool IsSelfOriginAlreadyReportedItem(const RoomItem& item,
...
bool IsSelfOriginItem(const RoomItem& item, const std::unordered_map<std::string, std::string>& metadata) {
    if (item.ap_player_id == 0 || item.ap_location_id == 0) {
        return false;
    }
    const auto slot_it = metadata.find("slot");
...
std::string RoomItemTrackerValue(const RoomItem& item, std::string_view fallback_value = {}) {
...
Event MakeRoomItemTrackerEvent(const RoomItem& item,
...
DomainDescriptorMap BuildDomainDescriptorMap(const MemoryProvider& provider) {
...
bool IsSupportedUnsignedSize(std::uint64_t size) {
    return size == 1 || size == 2 || size == 4 || size == 8;
}

std::vector<ReadSnapshotSpan> BuildCheckReadSnapshotSpans(const BridgeManifest& manifest,
...
void ReadCheckSnapshot(MemoryProvider& provider, std::vector<ReadSnapshotSpan>& spans) {
    for (auto& span : spans) {
...
std::optional<std::uint64_t> BytesToUnsigned(const std::byte* bytes, std::size_t size, Endianness endianness) {
...
std::optional<std::uint64_t> ReadUnsignedFromSnapshot(const std::vector<ReadSnapshotSpan>& spans,
...
RuntimeStatus ManifestBridgeSession::tick(const TickContext& context) {
    ...
    for (const auto& rule : manifest_.checks) {
        auto current = ReadUnsignedFromSnapshot(read_snapshot, rule);
...
bool RoomSynchronizedRuntimeSession::apply_room_item(const RoomItem& item, const TickContext& context) {
    ...
    applied_detail << "item_id=" << item.item_id
                   ...
                   << " mode=" << (uses_write_sequence ? "write_sequence" : "direct")
