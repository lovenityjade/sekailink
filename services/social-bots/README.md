...
- salons vocaux privés par lobby, créés à la demande et nettoyés après 15 minutes vides
- rapports de bugs avec bundles diagnostics privés conservés 30 jours
- export administratif des threads de bugs, discussions, screenshots et pièces jointes
...
### Export administratif des bugs

```bash
curl -X POST http://127.0.0.1:8095/discord/bug-reports/export \
  -H 'Content-Type: application/json' \
  -H 'X-SekaiLink-Bot-Key: CHANGE_ME' \
  -d '{"statuses":["OPEN","ONGOING"],"include_attachments":true}' \
  -o sekailink-bug-reports.zip
```
...
curl -X POST http://127.0.0.1:8095/ai/memory/ingest \
  -H 'Content-Type: application/json' \
  -H 'X-SekaiLink-Bot-Key: CHANGE_ME' \
  -d '{"directory":"/opt/sekailink/sekailink-client-plan"}'
```
