...
from .models import (
    ...
    DiscordBugReportRequest,
    DiscordBugReportExportRequest,
    DiscordFaqSyncRequest,
    DiscordLobbyVoiceCreateRequest,
    DiscordLobbyVoiceDeleteRequest,
    DiscordLobbyVoiceGrantRequest,
...
def create_app(service: BotService) -> FastAPI:
    ...
    @app.post("/discord/lobby-voice/grant", dependencies=[Depends(auth)])
    async def discord_lobby_voice_grant(payload: DiscordLobbyVoiceGrantRequest) -> dict:
        try:
            result = await service.discord.grant_lobby_voice(payload.lobby_id, payload.member_id)
        except Exception as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        return {"ok": True, **result}
    ...
    @app.post("/discord/bug-report", dependencies=[Depends(auth)])
    async def discord_bug_report(payload: DiscordBugReportRequest) -> dict:
        try:
            result = await service.discord.create_bug_report_thread(
                title=payload.title,
                ...
                app_info=payload.app_info or {},
                bundle_base64=payload.bundle_base64,
                bundle_manifest=payload.bundle_manifest or {},
                channel_id=payload.channel_id,
            )
        except Exception as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    @app.get("/discord/bug-reports/bundle/{report_id}", dependencies=[Depends(auth)])
    async def discord_bug_report_bundle(report_id: str):
        try:
            bundle_path = service.discord.get_bug_report_bundle(report_id)
        except Exception as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        return FileResponse(path=bundle_path, media_type="application/zip", filename=bundle_path.name)

    @app.post("/discord/bug-reports/export", dependencies=[Depends(auth)])
    async def discord_bug_reports_export(payload: DiscordBugReportExportRequest, background_tasks: BackgroundTasks):
        try:
            export_path = await service.discord.export_bug_reports(
