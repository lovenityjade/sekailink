...
GameServerTcpService::GameServerTcpService(GameSessionRegistry& registry,
                                           const GameServerAuthPolicy* auth_policy)
    : registry_(registry), auth_policy_(auth_policy) {}

GameServerTcpService::~GameServerTcpService() {
  stop();
}

bool GameServerTcpService::start(std::uint16_t port) {
  if (running_) {
    return false;
  }
#if defined(_WIN32)
...
void GameServerTcpService::stop() {
  running_ = false;
...
bool GameServerTcpService::serve_one() {
  ...
  try {
    response = handle_game_protocol_json(registry_, auth_policy_, nlohmann::json::parse(payload));
...
bool GameServerTcpService::serve_forever() {
  if (!running_ || listen_fd_ < 0) {
    return false;
  }
  while (running_) {
...
bool GameServerTcpService::start_background(std::uint16_t port) {
  if (!start(port)) {
    return false;
  }
...
void GameServerTcpService::join() {
  if (server_thread_.joinable()) {
    server_thread_.join();
  }
}

std::uint16_t GameServerTcpService::port() const {
  return bound_port_;
}

bool GameServerTcpService::running() const {
  return running_;
}

std::string game_tcp_send_json_line(const std::string& host, std::uint16_t port, const nlohmann::json& payload) {
#if defined(_WIN32)
  WSADATA wsa_data;
  if (WSAStartup(MAKEWORD(2, 2), &wsa_data) != 0) {
