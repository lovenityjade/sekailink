...
GenerationSlotRequest parse_generation_slot_request(const nlohmann::json& slot) {
  return {
      .slot_id = slot.at("slot_id").get<int>(),
      .user_id = slot.value("user_id", std::int64_t{}),
      .display_name = slot.value("display_name", std::string{}),
      .game_key = slot.at("game_key").get<std::string>(),
      .linkedworld_id = slot.at("linkedworld_id").get<std::string>(),
      .config_version_id = slot.value("config_version_id", std::int64_t{}),
      .config_snapshot = slot.value("config_snapshot", nlohmann::json::object()),
  };
}
...
nlohmann::json handle_generation_command(
  ...
  if (cmd == "submit_seed_package") {
    GenerationPackageRequest request;
    request.job_id = command.at("job_id").get<std::string>();
    request.room_id = command.at("room_id").get<std::string>();
    request.seed_id = command.at("seed_id").get<std::string>();
    request.rng_seed = command.value("rng_seed", request.seed_id);
    request.output_root = command.at("output_root").get<std::string>();

    for (const auto& slot : command.at("slots")) {
      request.slots.push_back(parse_generation_slot_request(slot));
    }

    std::vector<LinkedWorldGenerationSurface> surfaces;
    nlohmann::json load_errors = nlohmann::json::array();
    for (const auto& root : command.at("linkedworld_roots")) {
      std::string error;
      auto surface = load_linkedworld_generation_surface(root.get<std::string>(), &error);
      if (!surface.has_value()) {
        load_errors.push_back({
            {"root", root.get<std::string>()},
            {"error", error},
        });
        continue;
      }
    ...
    if (!load_errors.empty()) {
      return {
          {"ok", false},
          {"error", "linkedworld_load_failed"},
          {"load_errors", load_errors},
      };
    }

    const auto package = generate_seed_package_from_linkedworlds(request, surfaces);
    if (!package.ok) {
      return {
          {"ok", false},
          {"error", package.error},
    ...
    return {
        {"ok", true},
        {"package_dir", package.package_dir.string()},
