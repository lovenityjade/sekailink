...
class PokemonEmeraldClient(BizHawkClient):
    ...
    async def game_watcher(self, ctx: "BizHawkClientContext") -> None:
        ...
        goal = slot_data.get("goal", Goal.option_champion)

        if goal == Goal.option_champion:
            self.goal_flag = DEFEATED_WALLACE_FLAG
        elif goal == Goal.option_steven:
            self.goal_flag = DEFEATED_STEVEN_FLAG
        elif goal == Goal.option_norman:
            self.goal_flag = DEFEATED_NORMAN_FLAG
        elif goal == Goal.option_legendary_hunt:
            self.goal_flag = None

        if slot_data.get("remote_items", RemoteItems.option_false) == RemoteItems.option_true and not ctx.items_handling & 0b010:
        ...
        try:
            ...
            if slot_data.get("dexsanity", Toggle.option_false) == Toggle.option_true:
                # Read pokedex flags
                read_result = await bizhawk.guarded_read(
                    ctx.bizhawk_ctx,
            ...
            allowed_legendary_hunt = slot_data.get("allowed_legendary_hunt_encounters", []) or []
            ...
            if goal == Goal.option_legendary_hunt:
                # If legendary hunt doesn't require catching, add defeated legendaries to caught_legendaries
                if slot_data.get("legendary_hunt_catch", Toggle.option_false) == Toggle.option_false:
                ...
                for legendary, is_caught in caught_legendaries.items():
                    if is_caught and legendary in [LEGENDARY_NAMES[name.lower()] for name in allowed_legendary_hunt]:
                        num_caught += 1

                if num_caught >= int(slot_data.get("legendary_hunt_count", 0) or 0):
                    game_clear = True
