...
class MinishCapClient(BizHawkClient):
    ...
    async def game_watcher(self, ctx: "BizHawkClientContext") -> None:
        ...
        remote_items = ctx.slot_data.get("remote_items", Toggle.option_false)
        if remote_items == Toggle.option_true and not ctx.items_handling & 0b010:
        ...
        try:
            ...
            if not self.version_checked:
                ...
                multiworld_version = str(ctx.slot_data.get("version", "") or "")
                client_version = get_version()
                if multiworld_version and multiworld_version != client_version:
